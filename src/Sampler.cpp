...
struct AeSampler : Module {
	...
	//flag for thread-safe quick fix
	bool skipDraw = false;
...
struct SampleDisplay : TransparentWidget {
    ...
    void draw(NVGcontext *vg) override {
		...
		//thread-safe quick-fix
		if (module->skipDraw) {
			//reset flag and skip processing
			module->skipDraw = false;
			return;
		}
		...
		if(module->activeSample && module->activeSample->buffer) {
			...
			for (int i = 0; si && si->buffer && i < si->bufferLength; i++) {
				float x, y;
				
				//thread-safe quick fix
				if (module->skipDraw) {
					debug("SampleDisplay@draw => skipping draw routine at loop index %i ", i);
					module->skipDraw = false;
					break;
				}
